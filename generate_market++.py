...
train_dataloader2 = torch.utils.data.DataLoader(train_dataset, batch_size=opt.batchSize,
...
if __name__ == '__main__':
    ...
    train2_iter = enumerate(train_dataloader2)
    for data in tqdm.tqdm(train_dataloader):
        ...
        with torch.no_grad():
            ...
            padding  = torch.nn.ReflectionPad2d((8, 8, 16, 16))
            ...
            for repeat in range(2):
                try:
                    ...
                    train2_iter = enumerate(train_dataloader2)
                    _, data2 = train2_iter.__next__()
                Xa2 = data2['data']['images'].cuda()
                paths2 = data2['data']['path']
                Ae2 = netE(Xa2)
                A_tmp['textures'] =  0.5 * Ae['textures'] + 0.5 * Ae2['textures'][0:current_batchSize,:,:,:]
                diff = (A_tmp['textures'] - Ae['textures'])**2
                max_diff, _ = torch.max(torch.mean(diff, dim=(2,3)), dim=1)
                diff_score, diff_indices = torch.sort(max_diff, descending=True) # we select most different mixup
                for delta_azimuth in loop:
                    ...
                    for i in range(current_batchSize//2):
                        ...
                        single_mask  = mask[selected,:,:] # bg is black 0, fg is white 1
                        single_mask = makeup_hole(single_mask.unsqueeze(0))
                        blur_mask = gaussian_blur_mask(single_mask)
                        blur_mask = imresize(padding_mask(blur_mask))
                        ...
                        p = paths[selected]
                        old_id = os.path.basename(paths[selected]).split('_')[0]
                        new_id = os.path.basename(paths2[selected]).split('_')[0]
                        if int(old_id) < int(new_id):
                            dir_id = old_id+new_id
                        elif int(old_id) == int(new_id):
                            print('sample same identity.')
