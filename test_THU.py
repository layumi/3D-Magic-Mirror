...
from datasets.thuman2 import THUMan2Dataset
...
parser.add_argument('--dataroot', default='../ICON/data/thuman2_36views/', help='path to dataset root dir')
...
opt.nolpl = config['nolpl']
print(opt)

if torch.cuda.is_available():
    cudnn.benchmark = True

train_dataset = THUMan2Dataset('../ICON/data/thuman2_36views/', opt.imageSize, train=True, threshold=opt.threshold, bg = opt.bg, hmr = opt.hmr)
test_dataset = THUMan2Dataset('../ICON/data/thuman2_36views/', opt.imageSize, train=False, threshold=opt.threshold, bg = opt.bg, hmr = opt.hmr)
print('THuman2.0')
ratio = 2
...
if __name__ == '__main__':
    ...
    netE = AttributeEncoder(num_vertices=diffRender.num_vertices, vertices_init=diffRender.vertices_init, 
                            ...
                            coordconv = opt.coordconv, nolpl = opt.nolpl, 
                            norm = opt.norm, lpl = diffRender.vertices_laplacian_matrix) # height = 2 * width
    ...
    norm_dir = os.path.join(opt.outf, 'fid/norm')
    rec_norm_dir = os.path.join(opt.outf, 'fid/rec_norm')
    ...
    os.makedirs(rec_norm_dir, exist_ok=True)
    ...
    os.system('rm -r %s'%norm_dir)
    os.system('rm -r %s'%rec_norm_dir)
    # clean again 
    os.makedirs(ori_dir, exist_ok=True)
    os.makedirs(ori_mask_dir, exist_ok=True)
    os.makedirs(rec_dir, exist_ok=True)
    ...
    #for i, data in tqdm.tqdm(enumerate(train_dataloader)):
        ...
        with torch.no_grad():
            ...
            for i in range(len(paths)):
                ...
                ori_norm_path = os.path.join(norm_dir, image_name)
                output_ori_norm = to_pil_image(norms[i].detach().cpu())

                Xer_norm[i] *= Xer[i,3,:,:] # remove background
                rec_norm_path = os.path.join(rec_norm_dir, image_name)
                output_rec_norm = to_pil_image(Xer_norm[i].detach().cpu())

                X_all.extend([output_Xer, output_Xir, output_Xir2, output_Xer90, output_Xer270, output_Xa, output_ori_mask, output_rec_mask, output_ori_norm, output_rec_norm])
                path_all.extend([rec_path, inter_path, inter_path2, inter90_path, inter270_path, ori_path, ori_mask_path, rec_mask_path, ori_norm_path, rec_norm_path])
                norm_error.append(torch.mean((norms[i].detach().cpu()-Xer_norm[i].detach().cpu())**2))
