...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for i in range(1):
            ...
            # recon normals
            gtn_path = 'demo_single/%d_gtn_'%i+ image_name
            output_gtn = to_pil_image(norm.detach().cpu())
            ...
            print('norm loss: %.4f'% torch.mean((norm-Xer_norm.detach().cpu())**2))
            X_all.extend([output_All, output_en, output_gtn])
            path_all.extend([all_path, en_path, gtn_path])
